...
// TODO: CHANGE THESE 2 VARIABLES WITH THE ACTUAL PRODUCTION DATA AND DELETE THIS COMMENT
...
const addDataToDb = () => {
  this.validateFields();
  if (fieldsAreValid()) {
    console.log(getZipsServiceArea());
    if (getZipsServiceArea()[0] && getZipsServiceArea()[1]) { // Check if the zips are in the service area
      firebase
        .firestore()
        ...
        .add({
          ...
          location: `${window.location.href}${redirectUserToSite()}`,
          timestamp: new Date().toISOString()
        });
    }
    window.location.href = this.redirectUserToSite();
  }
};
